    ...
    match Query::new(grammar, source) {
        ...
        Err(e) => {
            ...
            Query::new(grammar, "").unwrap_or_else(|_| {
                panic!(
                    "Failed to create empty fallback query for Kotlin {}",
                    query_name
                )
            })
        }
...
impl LanguageSupport for Kotlin {
    ...
    fn binding_query(&self) -> Option<&Query> {
        Some(BINDING_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/bindings.scm"),
                "bindings",
            )
        }))
    }
    ...
    fn completion_query(&self) -> Option<&Query> {
        Some(COMPLETION_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/completion.scm"),
                "completion",
            )
        }))
    }
    ...
    fn reassignment_query(&self) -> Option<&Query> {
        Some(REASSIGNMENT_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/reassignments.scm"),
                "reassignments",
            )
        }))
    }
    ...
    fn identifier_query(&self) -> Option<&Query> {
        Some(IDENTIFIER_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/identifiers.scm"),
                "identifiers",
            )
        }))
    }
    ...
    fn export_query(&self) -> Option<&Query> {
        Some(EXPORT_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/exports.scm"),
                "exports",
            )
        }))
    }
    ...
    fn assignment_query(&self) -> Option<&Query> {
        Some(ASSIGNMENT_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/assignments.scm"),
                "assignments",
            )
        }))
    }
    ...
    fn destructure_query(&self) -> Option<&Query> {
        Some(DESTRUCTURE_QUERY.get_or_init(|| {
            compile_query(
                &self.grammar(),
                include_str!("../../queries/kotlin/destructures.scm"),
                "destructures",
            )
        }))
    }
