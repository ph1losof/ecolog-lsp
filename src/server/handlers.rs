use tower_lsp::lsp_types::{
    Hover, HoverParams, CompletionParams, CompletionItem, CompletionItemKind, 
    HoverContents, MarkupContent, MarkupKind, Documentation, SemanticTokensParams, 
    SemanticTokensResult, SemanticTokens, Diagnostic, DiagnosticSeverity, NumberOrString,
...
use std::path::Path;
use korni::{ParseOptions, Error as KorniError};
use serde_json::json;
...
pub async fn handle_hover(params: HoverParams, state: &ServerState) -> Option<Hover> {
    ...
    let (env_var_name, binding_name, hover_range, is_binding, binding_kind) =
        if let Some(reference) = state.document_manager.get_env_reference_cloned(uri, position) {
            ...
            (binding.env_var_name, Some(binding.binding_name), binding.binding_range, true, Some(binding.kind))
        } else if let Some(usage) = state.document_manager.get_binding_usage_cloned(uri, position) {
            let kind = state.document_manager.get_binding_kind_for_usage(uri, &usage.name);
            (usage.env_var_name, Some(usage.name), usage.range, true, kind)
        } else {
            return None;
        };
    ...
    let resolved_result = state.core.resolution.resolve(&env_var_name, &context, registry).await;
    ...
    if let Ok(Some(variable)) = resolved_result {
        ...
        let value = if should_mask {
            ...
            let source = variable.source.file_path()
        ...
        let mut markdown = if is_binding {
            ...
            if b_name == env_var_name {
                 format!("**`{}`**\n\n**Value**: `{}`\n**Source**: `{}`", env_var_name, value, source_str)
            } else {
                 format!("**`{}`** → **`{}`**\n\n**Value**: `{}`\n**Source**: `{}`", b_name, env_var_name, value, source_str)
            }
        } else {
        ...
        if let Some(crate::types::BindingKind::Object) = binding_kind {
             ...
             let msg = format!("**`{}`** → **`{}`**\n\n*Environment Object*", b_name, env_var_name);
...
pub async fn handle_completion(params: CompletionParams, state: &ServerState) -> Option<Vec<CompletionItem>> {
    let (is_strict, should_mask) = {
        ...
        (config.strict.completion, config.masking.should_mask_completion())
    };
    ...
    if let Ok(all_vars) = state.core.resolution.all_variables(&context, registry).await {
         // Using pre-fetched config value
         
         let mut masker = state.masker.lock().await;
         
          Some(all_vars.into_iter().map(|var| {
             let value = if should_mask {
                 let source = var.source.file_path()
                     .and_then(|p| p.strip_prefix(&context.workspace_root).ok())
                     .and_then(|p| p.to_str());
...
pub async fn handle_definition(params: tower_lsp::lsp_types::GotoDefinitionParams, state: &ServerState) -> Option<tower_lsp::lsp_types::GotoDefinitionResponse> {
    use tower_lsp::lsp_types::{Url, Range, Position, Location, GotoDefinitionResponse};
    ...
    let registry = &state.core.registry;
    if let Ok(Some(variable)) = state.core.resolution.resolve(&env_var_name, &context, registry).await {
...
pub async fn handle_semantic_tokens_full(params: SemanticTokensParams, state: &ServerState) -> Option<SemanticTokensResult> {
    let uri = &params.text_document.uri;

    let file_name = uri.to_file_path().ok()?.file_name()?.to_string_lossy().to_string();
...
pub async fn compute_diagnostics(uri: &tower_lsp::lsp_types::Url, state: &ServerState) -> Vec<Diagnostic> {
    ...
    let content = {
        let doc_ref = state.document_manager.get(uri);
        let Some(doc) = doc_ref else { return vec![]; };
        doc.content.clone()
    };

    // 2. Get references from BindingGraph (direct refs + symbols resolving to env vars)
    let (references, env_var_symbols): (Vec<crate::types::EnvReference>, Vec<(compact_str::CompactString, tower_lsp::lsp_types::Range)>) = {
        if let Some(graph) = state.document_manager.get_binding_graph(uri) {
            ...
            let symbols: Vec<_> = graph.symbols().iter()
                .filter_map(|s| {
    ...
    let file_path = if let Ok(p) = uri.to_file_path() { p } else { return vec![]; };
        ...
        for entry in entries {
             if let korni::Entry::Error(err) = entry {
                 let (msg, code, severity) = match &err {
                     KorniError::ForbiddenWhitespace { .. } => ("Forbidden whitespace", "EDF001", DiagnosticSeverity::ERROR),
                     KorniError::DoubleEquals { .. } => ("Double equals sign detected", "EDF002", DiagnosticSeverity::ERROR),
                     KorniError::Generic { message, .. } if message == "Empty key" => ("Empty key", "EDF003", DiagnosticSeverity::ERROR),
                     KorniError::InvalidKey { .. } => ("Invalid character in key", "EDF004", DiagnosticSeverity::ERROR),
                     KorniError::UnclosedQuote { .. } => ("Unclosed quote", "EDF005", DiagnosticSeverity::ERROR),
                     KorniError::InvalidUtf8 { .. } => ("Invalid UTF-8 sequence", "EDF006", DiagnosticSeverity::WARNING),
                     KorniError::Expected { .. } => ("Syntax error", "EDF999", DiagnosticSeverity::ERROR),
    ...
    if !is_env_file {
        ...
        if let Some(context) = context_opt {
            ...
            for reference in references {
                let resolved_result = state.core.resolution.resolve(&reference.name, &context, registry).await;
            ...
            for (env_name, range) in env_var_symbols {
                let resolved_result = state.core.resolution.resolve(&env_name, &context, registry).await;
...
fn get_line_col(content: &str, offset: usize) -> (u32, u32) {
    if offset >= content.len() { return (0, 0); }
     ...
     match params.command.as_str() {
         "ecolog.file.setActive" => {
             // Check if file source is enabled
             {
                 let config = state.config.get_config();
                 let config = config.read().await;
                 if !config.resolution.precedence.contains(&abundantis::config::SourcePrecedence::File) {
                     return Some(json!({ "error": "File source is not enabled in configuration" }));
                 }
             }

            // Arguments: file patterns as strings
            let patterns: Vec<String> = params.arguments
                .into_iter()
        ...
        "ecolog.listEnvVariables" => {
            ...
            match state.core.all_for_file(&root).await {
                Ok(vars) => {
                    let var_list: Vec<serde_json::Value> = vars.iter().map(|v| {
                        json!({
                            "name": v.key,
                ...
                Err(e) => {
                    Some(json!({ "error": format!("Failed to list variables: {}", e) }))
                }
            }
        }
         "ecolog.file.list" => {
