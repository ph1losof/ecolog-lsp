...
async fn main() {
    ...
    let config = config_manager.load_from_workspace(&root).await
    ...
    let masker = Arc::new(Mutex::new(
        shelter::Masker::new(shelter_config)
    ));
    ...
    let (service, socket) = LspService::new(|client| {
        LspServer::new_with_config(client, core, masker, config_arc)
    });
