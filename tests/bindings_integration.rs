...
use ecolog_lsp::server::handlers::{handle_hover, compute_diagnostics};
...
"#;
    ...
    state.document_manager.open(uri_js.clone(), "javascript".to_string(), js_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    let hover = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri_js.clone() },
...
"#;
    ...
    state.document_manager.open(uri_ts.clone(), "typescript".to_string(), ts_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    let hover = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri_ts.clone() },
            position: Position::new(2, 0),
        },
        work_done_progress_params: Default::default(),
    }, &state).await;
...
"#;
    ...
    state.document_manager.open(uri_ts_bracket.clone(), "typescript".to_string(), ts_bracket_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    let hover = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri_ts_bracket.clone() },
            position: Position::new(2, 0),
        },
        work_done_progress_params: Default::default(),
    }, &state).await;
...
"#;
    ...
    state.document_manager.open(uri_scope.clone(), "javascript".to_string(), scope_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    let hover = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri_scope.clone() },
            position: Position::new(4, 0),
        },
        work_done_progress_params: Default::default(),
    }, &state).await;

    assert!(hover.is_none(), "Scope Isolation Failed: Should not hover out-of-scope variable");
...
"#;
    ...
    state.document_manager.open(uri_alias.clone(), "javascript".to_string(), alias_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;

    // Hover on 'env' binding declaration (line 1, col 6)
    let hover_decl = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri_alias.clone() },
    ...
    assert!(hover_decl.is_some(), "Expected hover for object alias declaration");
    let hover_str = format!("{:?}", hover_decl.unwrap());
    assert!(hover_str.contains("Environment Object"), "Detailed message should indicate Environment Object, got: {}", hover_str);
    assert!(!hover_str.contains("(undefined)"), "Should not show (undefined) for object alias");
...
async fn test_destructuring_diagnostics() {
    ...
    state.document_manager.open(uri_direct.clone(), "javascript".to_string(), js_direct_content.to_string(), 0).await;
    ...
    assert!(!diagnostics_direct.is_empty(), "Should have diagnostic for undefined UNDEFINED_VAR in direct access");
    assert!(diagnostics_direct.iter().any(|d| d.message.contains("UNDEFINED_VAR")),
    ...
    state.document_manager.open(uri_destruct.clone(), "javascript".to_string(), js_destruct_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    ...
    assert!(!diagnostics_destruct.is_empty(), "Should have diagnostic for undefined UNDEFINED_VAR in destructuring");
    assert!(diagnostics_destruct.iter().any(|d| d.message.contains("UNDEFINED_VAR")),
            "Diagnostic should mention UNDEFINED_VAR");
    ...
    state.document_manager.open(uri_renamed.clone(), "javascript".to_string(), js_renamed_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;

    let diagnostics_renamed = compute_diagnostics(&uri_renamed, &state).await;
    println!("Renamed destructuring diagnostics: {:?}", diagnostics_renamed);
    assert!(!diagnostics_renamed.is_empty(), "Should have diagnostic for undefined UNDEFINED_VAR in renamed destructuring");
    assert!(diagnostics_renamed.iter().any(|d| d.message.contains("UNDEFINED_VAR")),
            "Diagnostic should mention UNDEFINED_VAR");
    ...
    state.document_manager.open(uri_defined.clone(), "javascript".to_string(), js_defined_content.to_string(), 0).await;
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;

    let diagnostics_defined = compute_diagnostics(&uri_defined, &state).await;
    println!("Defined var diagnostics: {:?}", diagnostics_defined);
    assert!(diagnostics_defined.is_empty() || !diagnostics_defined.iter().any(|d| d.message.contains("DB_URL")),
