...
use tower_lsp::lsp_types::{HoverParams, TextDocumentPositionParams, TextDocumentIdentifier, Position, CompletionParams, CompletionContext, CompletionTriggerKind};
...
async fn test_py_hover_environ_getitem() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), "import os\nval = os.environ['DB_URL']".to_string(), 0).await;
...
async fn test_py_hover_environ_get() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), "import os\nval = os.environ.get('API_KEY')".to_string(), 0).await;
...
async fn test_py_hover_os_getenv() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), "import os\nval = os.getenv('PORT')".to_string(), 0).await;
...
async fn test_py_hover_from_import() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), "from os import environ\nx = environ['DEBUG']".to_string(), 0).await;
...
async fn test_py_hover_walrus_operator_environ_get() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), 
        "import os\nif (db_url := os.environ.get('DB_URL')):\n  print(db_url)".to_string(), 0).await;
...
async fn test_py_hover_walrus_operator_getenv() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), 
        "import os\nif (api_key := os.getenv('API_KEY')):\n  print(api_key)".to_string(), 0).await;
...
async fn test_py_diagnostics_walrus_undefined() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), 
        "import os\nif (missing := os.environ.get('MISSING_VAR')):\n  print(missing)".to_string(), 0).await;
