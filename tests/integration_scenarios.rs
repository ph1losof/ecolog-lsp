...
async fn test_scenario_multiple_env_files() {
    ...
    fixture.state.document_manager.open(uri.clone(), "javascript".to_string(), "process.env.PORT".to_string(), 0).await;
    ...
    assert!(h_str.contains("9090"), "Expected override value 9090, got: {}", h_str);
    
    // Hover NEW_VAR
    fixture.state.document_manager.change(&uri, vec![TextDocumentContentChangeEvent {
        range: None,
        range_length: None,
        text: "process.env.NEW_VAR".to_string(),
    }], 1).await;
    
    let hover_new = handle_hover(HoverParams {
        text_document_position_params: TextDocumentPositionParams {
...
async fn test_scenario_env_syntax_diagnostics() {
    ...
    fixture.state.document_manager.open(uri.clone(), "plaintext".to_string(), "BAD LINE".to_string(), 0).await;
...
async fn test_scenario_quoted_values() {
    ...
    fixture.state.document_manager.open(uri.clone(), "python".to_string(), "os.environ['QUOTED']".to_string(), 0).await;
...
async fn test_scenario_commented_env() {
    ...
    fixture.state.document_manager.open(uri.clone(), "javascript".to_string(), "process.env.IGNORE_ME".to_string(), 0).await;
    ...
    assert!(hover.is_none(), "Hover should return None for undefined variables");

    // Verify diagnostic is generated for undefined var
    let diags = compute_diagnostics(&uri, &fixture.state).await;
    assert!(diags.iter().any(|d| d.message.contains("not defined")), "Should have diagnostic for undefined var");
}
...
async fn test_scenario_empty_value() {
    ...
    fixture.state.document_manager.open(uri.clone(), "javascript".to_string(), "process.env.EMPTY".to_string(), 0).await;
    ...
    assert!(hover.is_some(), "Hover should find EMPTY variable with empty value");
    // Value should be empty string
    let h_str = format!("{:?}", hover.unwrap());
    assert!(h_str.contains("Value**: ``") || h_str.contains("Value**: \"\""),
        "Expected empty value, got: {}", h_str);
}
...
async fn test_scenario_multiline_value() {
    ...
    fixture.state.document_manager.open(uri.clone(), "rust".to_string(), "std::env::var(\"MULTI\")".to_string(), 0).await;
...
async fn test_scenario_forbidden_whitespace() {
    ...
    fixture.state.document_manager.open(uri.clone(), "plaintext".to_string(), "KEY =VALUE".to_string(), 0).await;
    
    let diags = compute_diagnostics(&uri, &fixture.state).await;
    if !diags.is_empty() {
         assert!(diags[0].message.contains("whitespace") || diags[0].message.contains("Unexpected") || diags[0].message.contains("Invalid"));
    }
}
...
async fn test_masking_hover() {
    ...
    fixture.state.document_manager.open(uri.clone(), "javascript".to_string(), "process.env.API_KEY".to_string(), 0).await;
    ...
    assert!(!h_str.contains("secret_key"),
...
async fn test_case_sensitivity() {
    ...
    assert!(hover.is_none(), "db_url (lowercase) should not match DB_URL");
}
...
async fn test_active_file_selection() {
    ...
    fixture.state.document_manager.open(uri.clone(), "javascript".to_string(), "process.env.MODE".to_string(), 0).await;
