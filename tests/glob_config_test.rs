...
async fn test_glob_config_semantic_tokens() {
    ...
    state.document_manager.open(uri.clone(), "plaintext".to_string(), content, 0).await;
    ...
    assert!(result.is_some(), "Expected semantic tokens for custom env file pattern");
    ...
    state.document_manager.open(uri_other.clone(), "plaintext".to_string(), "SOME=VAL".to_string(), 0).await;
    
    let params_other = SemanticTokensParams {
        text_document: TextDocumentIdentifier { uri: uri_other.clone() },
        work_done_progress_params: Default::default(),
        partial_result_params: Default::default(),
    };
    let result_other = handle_semantic_tokens_full(params_other, &state).await;
    assert!(result_other.is_none(), "Should not return tokens for non-matching file");
    ...
    let files = val.get("files").expect("Expected files list").as_array().expect("Expected array");
    
    // Check if our custom file is in the list
    let found = files.iter().any(|v| v.as_str() == Some("production.env.custom"));
    assert!(found, "production.env.custom should be found by ecolog.file.list");
    ...
    let result_retry = ecolog_lsp::server::handlers::handle_execute_command(params_retry, &state).await;
    ...
    assert!(!found_other, "other.txt should NOT be found by ecolog.file.list");
